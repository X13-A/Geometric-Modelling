...
//		myHalfedge* e = input_mesh->faces[i]->adjacent_halfedge;
//		do {
//			cx += (GLfloat)e->source->point->X;
//			cy += (GLfloat)e->source->point->Y;
//			cz += (GLfloat)e->source->point->Z;
//			count++;
//			e = e->next;
//		} while (e != input_mesh->faces[i]->adjacent_halfedge);
//		cx /= count;
//		cy /= count;
//		cz /= count;
//
//		// Calculate the end point of the normal line
//		GLfloat nx = cx + (GLfloat)input_mesh->faces[i]->normal->dX / 20.0f;
//		GLfloat ny = cy + (GLfloat)input_mesh->faces[i]->normal->dY / 20.0f;
//		GLfloat nz = cz + (GLfloat)input_mesh->faces[i]->normal->dZ / 20.0f;
//
//		// Add the centroid and the end point of the normal line to the verts_and_normals vector
//		verts_and_normals.push_back(cx);
//		verts_and_normals.push_back(cy);
//		verts_and_normals.push_back(cz);
//		verts_and_normals.push_back(nx);
//		verts_and_normals.push_back(ny);
//		verts_and_normals.push_back(nz);
//	}
//
//
...
void initInterface(int argc, char* argv[])
{
	...
	glutCreateWindow("My IN4I12 Application!");
